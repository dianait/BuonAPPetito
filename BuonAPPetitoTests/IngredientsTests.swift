...
final class IngredientsTests: XCTestCase {
    ...
    func testAddIngredient() throws {
        ...
        order.addIngredient(ing: ingredients[1], pizza: order.pizzas[0])
        XCTAssertEqual(order.pizzas[0].ingredients.count, 2)
        XCTAssertEqual(order.pizzas[0].ingredients.first, ingredients[0])
        XCTAssertEqual(order.pizzas[0].ingredients[1], ingredients[1])
    }

    func testRemoveIngredient() throws {
        order.addIngredient(ing: ingredients[0], pizza: order.pizzas[0])
        order.addIngredient(ing: ingredients[1], pizza: order.pizzas[0])
        order.addIngredient(ing: ingredients[2], pizza: order.pizzas[0])
        order.removeIngredient(ing: ingredients[0], pizza: order.pizzas[0])
        XCTAssertEqual(order.pizzas[0].ingredients.count, 2)
    }
    ...
    func testRemoveIngredientsToNone() throws {
        ...
        order.addIngredient(ing: ingredients[2], pizza: order.pizzas[0])
        order.removeIngredient(ing: ingredients[0], pizza: order.pizzas[0])
        order.removeIngredient(ing: ingredients[1], pizza: order.pizzas[0])
        order.removeIngredient(ing: ingredients[2], pizza: order.pizzas[0])
        XCTAssertEqual(order.pizzas[0].ingredients, [])
        XCTAssertEqual(order.pizzas[0].ingredients.count, 0)
    }

    func testResetCustomPizza() throws {
        order.addIngredient(ing: ingredients[0], pizza: order.pizzas[2])
        order.addIngredient(ing: ingredients[1], pizza: order.pizzas[2])
